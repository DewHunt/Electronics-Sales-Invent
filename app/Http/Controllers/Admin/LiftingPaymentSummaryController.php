        ...
        else
        {
            ...
            $liftingPaymentSummaries = DB::table('view_lifting_payment_summary')
                ...
                ->orWhere(function($query) use($year,$vendor){
                    ...
                    if (@$vendor)
                    {
                        $query->whereIn('vendors.id',$vendor);
                    }
                })
                ->groupBy('view_lifting_payment_summary.vendorId','view_lifting_payment_summary.vendorName')
                ->unionAll($monthlyvendorStatements)
                ->unionAll($previousvendorStatements)
                ->orderBy('vendorId')
                ->get();
        }

    	return view('admin.liftingPaymentSummary.index')->with(compact('title','searchFormLink','printFormLink','print','vendors','vendor','year','month'));
    }
}
